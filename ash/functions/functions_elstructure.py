...
###################################
# Processing of Loewdin reduced MO analysis from ORCA output
# Functions get_loewdin_table, convert_loewdin_text_to_dataframe, select_contr and plot_dataframe_table
# were written by Nico Spiller (taken from https://gitlab.gwdg.de/orca-helpers/orca-helpers/-/tree/master/orca_helpers/output_files/loewdin_plotter)
def get_loewdin_table(file):
...
def convert_loewdin_text_to_dataframe(block, nmax=None, only_occ=False):
...
def select_contr(df, atom2index, mos=[], atoms=[], subshells=[], collapse=0):
    ...
    if collapse == 1:
        ...
        df = df.T.groupby(level=(0, 1)).sum().T
...
def plot_dataframe_table(df, path=None, annot=True, s=1.0):
    '''Plot a single DataFrame'''
    import matplotlib.pylab as plt
    import seaborn as sns
    y, x = [i/2/s for i in df.shape]
...
# Function to automate localized orbital analysis from ORCA output
def localized_orbital_analysis(theory=None, fragment=None, 
                               angmom='d', threshold=70, atomlist=None,
                               plot_table=True, create_cubefiles=True, gridval=60,
                               debug=False):

    if theory is None or fragment is None:
        print("Error: please provide a valid ASH Theory and Fragment")
        ashexit()
    if atomlist is None:
        print("Error: please provide a list of atoms to analyze via atomlist keyword")
        ashexit()

    # Run theory to get orbitals
    print("Running theory to get orbitals")

    Singlepoint(theory=theory, fragment=fragment)


    # ORCA noiter job to get Löwdin decomposition of localized orbitals
    dummytheory=ORCATheory(orcasimpleinput=theory.orcasimpleinput+' noiter normalprint',
                moreadfile=theory.filename+'.loc', filename="orca_noiter")
    Singlepoint(theory=dummytheory, fragment=fragment)
    ...
    # reduce dataframes
    print("Selecting localized orbitals...")
    for i, df in enumerate(dfarr):
        print('INFO processing block: {}'.format(i))
        dfarr[i] = select_contr(df, atom2index, mos=[], atoms=atomlist, subshells=[], collapse=1)
    dfarr_a= dfarr[0]
    dfarr_b= dfarr[1]
    if debug:
        print("dfarr_a:", dfarr_a)
        print("dfarr_b:", dfarr_b)

    # Getting column
    col_a = next(c for c in dfarr_a.columns if c[1] == angmom)
    col_b = next(c for c in dfarr_b.columns if c[1] == angmom)

    # Select d-orbitals for alpha and beta
    indices_a = dfarr_a.index[dfarr_a[col_a] > threshold].tolist()
    indices_b = dfarr_b.index[dfarr_b[col_b] > threshold].tolist()

    print()
    print()
    # Print relevant MOs
    print("Found these localized MOs:")
    print("alpha:")
    for i in indices_a:
        perc = dfarr_a.loc[i, col_a]
    ...
    if plot_table:
        ...
        plot_dataframe_table(dfarr_a, path=output, annot=True, s=1.0)
        output="table_b.png"
        print('INFO plotting: {}'.format(output))
        plot_dataframe_table(dfarr_b, path=output, annot=True, s=1.0)

    #Create cubefiles for selected MO-numbers
    if create_cubefiles:
        print("\n\n Now creating cubefiles for selected MO-numbers")
        for MONUMBER in indices_a:
            print(f"Now plotting alpha MO {MONUMBER}")
            run_orca_plot("orca.loc", "mo", gridvalue=gridval, mo_operator=0, mo_number=MONUMBER)
            os.rename(f"orca.mo{MONUMBER}a.cube", f"orcaloccalc_alpha_mo{MONUMBER}.cube") #Renaming cube file
        print()
        for MONUMBER in indices_b:
            print(f"Now plotting beta MO {MONUMBER}")
            run_orca_plot("orca.loc", "mo", gridvalue=gridval, mo_operator=1, mo_number=MONUMBER)
            os.rename(f"orca.mo{MONUMBER}b.cube", f"orcaloccalc_beta_mo{MONUMBER}.cube") #Renaming cube file