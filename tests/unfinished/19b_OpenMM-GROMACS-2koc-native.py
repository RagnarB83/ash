...
openmmobject_native = OpenMMTheory(GROMACSfiles=True, gromacstopfile=topfile, grofile=grofile, gromacstopdir=gromacstopdir,
               periodic=True, applyconstraints=True, use_parmed=False, do_energy_decomposition=True, dispersion_correction=True, periodic_nonbonded_cutoff=9,
                ewalderrortolerance=5e-5)

result = Singlepoint(theory=openmmobject_native, fragment=frag)
energy_native = result.energy
...
print("Calculated MM Bond energy:", openmmobject_native.energy_components['Bond']._value)
assert abs(float(openmmobject_native.energy_components['Bond']._value)-chosen_ref['Bond']) < threshold, "MM Bond Energy-error above threshold"

#Angle energy is sum of Angle+Urey-Bradley+Impropers+CMAP
print("Reference MM Angle energy:", chosen_ref['Angle'])
print("Calculated MM Angle energy:", openmmobject_native.energy_components['Angle'])
assert abs(float(openmmobject_native.energy_components['Angle']._value)-chosen_ref['Angle']) < threshold, "MM Angle Energy-error above threshold"

print("Reference MM Dihedral energy:", chosen_ref['Dihedral'])
print("Calculated MM Dihedral energy:", openmmobject_native.energy_components['Dihedrals']._value)
assert abs(float(openmmobject_native.energy_components['Dihedrals']._value)-chosen_ref['Dihedral']) < threshold, "MM Dihedral Energy-error above threshold"

print("Reference MM Nonbonded energy:", chosen_ref['Nonbonded'])
print("Calculated MM Nonbonded energy:", openmmobject_native.energy_components['Nonbonded']._value)
assert abs(float(openmmobject_native.energy_components['Nonbonded']._value)-chosen_ref['Nonbonded']) < threshold, "MM Nonbonded Energy-error above threshold"

print("Reference MM Total energy:", chosen_ref['Total'])
print("Calculated MM Sum Total energy:", openmmobject_native.energy_components['Sum'])
assert abs(openmmobject_native.energy_components['Sum']-chosen_ref['Total']) < threshold, "MM Total Energy-error above threshold"
print("")
