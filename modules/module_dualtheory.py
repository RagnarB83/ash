...
class DualTheory:
    ...
    def run(self, current_coords=None, elems=None, Grad=False, PC=False, numcores=None, label=None, charge=None, mult=None, run_both_theories=False ):
        ...
        if Grad == True:
            T1_energy, T1_grad = self.theory1.run(current_coords=current_coords, elems=elems, PC=PC, numcores=numcores, Grad=True, charge=charge, mult=mult)
            if run_both_theories == True:
                T2_energy, T2_grad = self.theory2.run(current_coords=current_coords, elems=elems, PC=PC, numcores=numcores, Grad=True, charge=charge, mult=mult)
            ...
            T1_energy = self.theory1.run(current_coords=current_coords, elems=elems, PC=PC, numcores=numcores, Grad=False, charge=charge, mult=mult)
            if run_both_theories == True:
                T2_energy = self.theory2.run(current_coords=current_coords, elems=elems, PC=PC, numcores=numcores, Grad=False, charge=charge, mult=mult)
                #Update gradient correction if both theories were calculated
                self.correction(Grad,label,T2_E=T2_energy,T1_E=T1_energy,T2_G=T2_grad,T1_G=T1_grad)
