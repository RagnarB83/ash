...
#Calculate Raman activiities from masses, (mass-weighted) eigenvectors and polarizability derivative array
def calc_Raman_activities(hessmasses,evectors,polarizability_derivs):
    ...
    #Finalizing polarizability derivative: making one big matrix
    A_der = np.zeros( (hesslength,9) )
    print("A_der shape", A_der.shape)
    print("A_der:", A_der)
    ...
    beta_alpha2    = np.zeros(hesslength)
    ramint_linear  = np.zeros(hesslength)
    depol_linear  = np.zeros(hesslength)
    for i in range(hesslength):
    ...
    pc_bohr2angstroms=0.52917721067
    pc_au2amu=0.00054857990907
    ...
    raman_conv=3.57122
    print("raman_conv:", raman_conv)
    for i in range(hesslength):
        print("-------------")
        print("alpha*raman_conv", alpha[i]*alpha[i]*raman_conv)
        print("beta_alpha2*raman_conv", beta_alpha2[i]*raman_conv)
        print("ramint_linear*raman_conv", ramint_linear[i]*raman_conv)
        print("depol_linear", depol_linear[i])
