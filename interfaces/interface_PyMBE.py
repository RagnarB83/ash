...
class PyMBETheory:
    def __init__(self, pymbedir=None, filename='pymbe_', printlevel=2,
        ...
        if pymbeinput is None:
            print("PyMBTheory requires a pymbinput keyword")
            ashexit()

        if pymbedir == None:
            print(BC.WARNING, "No pymbedir argument passed to MRCCTheory. Attempting to find pymbedir variable inside settings_ash", BC.END)
            try:
                print("settings_ash.settings_dict:", settings_ash.settings_dict)
                self.mrccdir=settings_ash.settings_dict["pymbedir"]
            except:
